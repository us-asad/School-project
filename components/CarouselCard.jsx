...
export default function CarouselCard({ image, alt, title, description, link }) {
	return (
		<div className="h-[250px] sm:h-[360px] md:h-[440px] lg:h-[540px] xl:h-[600px]">
			...
			<div className="absolute top-[15%] sm:top-[30%] md:left-32 md:right-32 w-full md:px-0 px-16 md:w-auto flex flex-col md:items-start items-center z-20 text-white">
				...
				<Link href={link.url}>
					<a className="text-sm md:text-2xl px-5 py-3 md:px-7 md:py-4 bg-customBlue text-white mt-5 md:mt-10 rounded-full">{link.name}</a>
				</Link>
			</div>
		</div>
	);
